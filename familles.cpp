...
bool Familles::ajouter(){
    ...
    QSqlQuery query; //La classe QSqlQuery fournit un moyen d'exécuter et de manipuler des instructions SQL
    QString id_string= QString::number(id_famille);
           query.prepare("INSERT INTO Familles  (id_famille, nom, prenom,nbr,numero,loca) "
                        "VALUES (:id, :forename, :surname,:nbr,:numero,:localisation)");//compilation de l'instruction
...
QSqlQueryModel *Familles::trinbr()
{
   QSqlQueryModel * model= new QSqlQueryModel();
    QSqlQuery *q = new QSqlQuery();

    q->prepare("SELECT * FROM familles ORDER BY nbr ");
    q->exec();
    model->setQuery(*q);
    return model;
...
int Familles::nombre()
{
  QSqlQueryModel* model1=new QSqlQueryModel();
    model1->setQuery("SELECT * FROM familles");

  return model1->rowCount();
